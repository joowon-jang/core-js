...
// data: 펼쳐서 출력할 객체, depth: 몇중첩인지(객체안에 객체안에 객체...)
function print(data, depth = 1) {
  // if: data가 객체인지 확인
  if (typeof data === 'object') {
    // for: data(객체)를 순회하면서 key와 value를 가져옴
    for (let keyValue of Object.entries(data)) {
      ...
      if (typeof value === 'object') {
        // 객체의 depth(중첩단계)에 따라 앞쪽 공백 + key를 출력 (value는 객체니까 일단 key만 출력하고 다음 줄에서 value(객체)의 key와 value를 함께 출력할거임)
        console.log('\t'.repeat(depth) + key, ' : ');
        // value가 객체일 때 실행이니까, value(객체안의 객체)를 펼쳐서 출력하기위해 print 함수에 value를 인자로 전달해서 호출
        // 중첩단계(객체안에 객체)가 깊어지므로 depth + 1
        print(value, depth + 1);
      } else {
        // value가 객체가 아닐때에는 key와 value를 모두 출력
